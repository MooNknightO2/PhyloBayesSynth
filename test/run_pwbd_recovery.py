...
N_MOVE = 25
SEED = 2026
OUT_DIR = Path("data/generated/pwbd_smc_recovery")
# ====================================================================

print("=" * 60)
print(f"PWBD Recovery Experiment")
print(f"  TRUE: {TRUE_EXPR_STR}")
print(f"  {N_TIPS} tips x {N_TREES} trees")
print(f"  SMC: {N_PARTICLES} particles, {N_MOVE} moves/round")
print("=" * 60)
...
for i in range(N_TREES):
    ...
    if not out_path.exists():
        ...
        print(f"  Gen tree {i}: seed={seed_i}")
...
def ll_wrap(model, td):
    return evaluate_likelihood(Expression("NoisyPhylo", [], [noise, model]), td)

true_cum = sum(ll_wrap(true_expr, td) for td in observations)
print(f"\nTrue model cum-logL: {true_cum:.2f}")

# Step 3: Single-tree MCMC (first tree) as quick check
print(f"\n--- Quick MCMC on tree 0 (500 iters) ---")
np.random.seed(SEED)
...
if r1["best_per_model"].get("PWBD"):
    expr_p, ll_p = r1["best_per_model"]["PWBD"]
    print(f"  Best PWBD: {ll_p:.4f} {expr_p}")
...
print(f"True cum-logL: {true_cum:.2f}")
